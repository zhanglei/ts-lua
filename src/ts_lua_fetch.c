...
ts_lua_fetch(lua_State *L)
{
    TSCont              contp;
    http_fetcher        *fch;
    const char          *url, *value, *key;
    size_t              len, val_len, key_len;
    ...
    /* option */
    fch = ts_http_fetcher_create(contp, &client_addr, 0);
    ...
    fctx = (ts_lua_fetch_ctx*)TSmalloc(sizeof(ts_lua_fetch_ctx));
